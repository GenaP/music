...
<center> <a href="<?php echo $_SERVER['PHP_SELF']?>?lang=ukr&member=<?php echo $member?>">ukr</a> | <a href="<?php echo $_SERVER['PHP_SELF']?>?lang=eng&member=<?php echo $member?>">eng</a></center>
  <hr/>
<div class="main">
	<div class="user_content">
	   <div class="ava-block"><center><b><?php echo $member;?></b><?php if ($user==$member) {echo "(it's
	   you)";}?></center>
		<img src="<?php echo $avatar;?>"  alt="no avatar" class="ava"/><br/>
		<?php if ($user==$member) 
	   ...
	   <div class="user_info"> <?=$lang['name']?>: <?php echo "".$name." ".$surname."";?><br/>
	   			   <?=$lang['status']?>: <?php echo $status;?><br/> 
	   			   <?=$lang['date_of_reg']?>: <?php echo $reg_date; ?><br/>
	   			   <?php if (isset($_SESSION['user_nick'])) {echo"E-mail: $email<br/>";}?>
	   			   <?=$lang['last_visit']?>: <?php echo $last_date;?><br/><hr/>
	   			  <?php if ($user==$member) {echo"<a class=edit_b onclick=javascript:showElement('edit')> ".$lang['edit_my_profile']."</a><br>
	...
	<input type=hidden name=id value=$rows->id><hr/><center>
        <input type=submit value= Edit style=margin-top:6px;></center>
	   			  ...
	   			  <a class=show onclick=javascript:showElement('order-p')>".$lang['delete_my_profile']."</a><br>
...
</div><br/><br/><hr/><br/><h2><i>Articles of this user</i></h2><br/>
<?php
try {
	...
	while ($rows=$prepare->fetch(PDO::FETCH_OBJ)) {
		$output[$rows->id] = '<h3>'.$rows->{"{$user_lang}.title"}.'</h3>--------------------------------------<br/><br/>
	  		Created '.$rows->date.' by <a href="profile.php?member='.$rows->author.'">'.$rows->author.'</a><br/><br/>
			'.textFunc($rows->{"{$user_lang}.text"},149).'... <br/><br/><a href="index.php?id='.$rows->id.'">Read more</a>';
